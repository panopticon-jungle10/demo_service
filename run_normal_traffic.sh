...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  정상 트래픽 부하 테스트 시작${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
while getopts "u:s:t:h" opt; do
  case $opt in
    ...
    h)
      ...
      echo "  -u: 동시 사용자 수 (기본값: 50)"
      echo "  -s: 사용자 증가 속도 (기본값: 5)"
      echo "  -t: 실행 시간 (기본값: 10m)"
      echo ""
      echo "예시: $0 -u 100 -s 10 -t 30m"
      exit 0
      ;;
    \?)
...
locust -f locust_normal_traffic.py \
    ...
    --run-time "${RUN_TIME}" \
    --headless \
    --html=locust_normal_traffic_report.html

echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  테스트 완료!${NC}"
echo -e "${GREEN}========================================${NC}"
echo -e "보고서: ${YELLOW}locust_normal_traffic_report.html${NC}"
